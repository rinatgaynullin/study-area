...
const findNode = (doc: unknown, type: string): JsonNode | undefined => {
  ...
  return (node?.content ?? []).reduce<JsonNode | undefined>(
    (found, child) => found ?? findNode(child, type),
    undefined,
  );
};

const findFormulaPos = (instance: RichEditorCore): number | null => {
