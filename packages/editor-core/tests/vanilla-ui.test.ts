...
describe('ванильная оболочка редактора', () => {
  ...
  it('пересобирает подписи тулбара при смене локали', () => {
    // Встроенная русская таблица — это запасной вариант, остальные языки хост
    // передаёт сам; готовый английский набор для этого и экспортируется.
    const editor = mountEditor({ messages: { en } });
