...
const keyLabel = (key: string): string => (key.length === 1 ? key.toUpperCase() : key);

/** «Mod-B» → «⌘B» на Mac и «Ctrl+B» везде ещё. */
export const formatShortcut = (shortcut: string, mac: boolean = isMacLike()): string => {
...
export const ariaKeyshortcuts = (shortcut: string, mac: boolean = isMacLike()): string => {
  const parts = shortcut.split('-');
  const key = keyLabel(parts.pop() ?? '');
